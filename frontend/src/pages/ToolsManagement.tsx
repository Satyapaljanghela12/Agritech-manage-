...
import { ToolEquipment } from '../types';
...
export const ToolsManagement = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const payload = {
        ...
        lastMaintenanceDate: formData.last_maintenance_date || null,
        nextMaintenanceDate: formData.next_maintenance_date || null,
        notes: formData.notes,
      };

      if (editingTool) {
        await api.updateTool(editingTool._id, payload);
      } else {
  ...
  const handleEdit = (tool: ToolEquipment) => {
    ...
    setFormData({
      ...
      purchase_cost: String(tool.purchaseCost),
      condition: tool.condition,
      last_maintenance_date: tool.lastMaintenanceDate || '',
      next_maintenance_date: tool.nextMaintenanceDate || '',
      notes: tool.notes || '',
    });
    setShowForm(true);
  };
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {tools.map((tool) => {
          const needsMaintenance =
            ...
            new Date(tool.nextMaintenanceDate) <= thirtyDaysLater;

          return (
            <div key={tool._id} className="bg-white rounded-xl p-6 border border-gray-200">
