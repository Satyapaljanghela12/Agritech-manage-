...
export const Dashboard = ({ onNavigate }: DashboardProps = {}) => {
  ...
  const loadDashboardData = async () => {
    ...
    try {
      ...
      const totalLand = landData.reduce((sum: number, land: any) => sum + Number(land.area), 0) || 0;
      const activeCrops = cropsData.filter((c: any) => ['planted', 'growing'].includes(c.status)).length || 0;

      const today = new Date();
      const thirtyDaysLater = new Date(today.getTime() + 30 * 24 * 60 * 60 * 1000);
      const upcomingHarvests =
        cropsData.filter((crop: any) => {
          const harvestDate = new Date(crop.expectedHarvestDate);
          return harvestDate >= today && harvestDate <= thirtyDaysLater;
        }).length || 0;

      const lowStockItems =
        inventoryData.filter((item: any) => item.quantity <= item.alertLevel).length || 0;

      const maintenanceDue =
        toolsData.filter((tool: any) => {
      ...
      const expenses = financialData.filter((r: any) => r.type === 'expense');
      const revenue = financialData.filter((r: any) => r.type === 'revenue');
      
      const totalExpenses = expenses.reduce((sum: number, exp: any) => sum + Number(exp.amount), 0) || 0;
      const totalRevenue = revenue.reduce((sum: number, rev: any) => sum + Number(rev.amount), 0) || 0;
