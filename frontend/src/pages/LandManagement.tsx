...
export const LandManagement = () => {
  ...
  const loadParcels = async () => {
    ...
    try {
      const data = await api.getLandParcels();
      setParcels(data || []);
    } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingParcel) {
        await api.updateLandParcel(editingParcel._id, payload);
      } else {
        await api.createLandParcel(payload);
      }

      setShowForm(false);
      setEditingParcel(null);
      setFormData({ name: '', area: '', soilType: '', location: '', notes: '' });
      loadParcels();
    } catch (error) {
